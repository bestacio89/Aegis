...
public sealed class ApiConsistencyEvaluator : BaseArchitectureEvaluator, IScopedDependency
{
    ...
    private IEnumerable<ArchitectureEvaluatorResult> EvaluateRoute(string file, string route)
    {
        var output = new List<ArchitectureEvaluatorResult>();

        if (_policy.EnforceLowercaseRoutes)
        {
            var hasUppercase = route.Any(char.IsUpper);
            output.Add(new ArchitectureEvaluatorResult(Name, file)
            {
                Category = "Casing",
    ...
    private IEnumerable<ArchitectureEvaluatorResult> EvaluateEndpoint(string file, string method, string path)
    {
        var output = new List<ArchitectureEvaluatorResult>();
