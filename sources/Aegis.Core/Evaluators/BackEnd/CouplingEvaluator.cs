...
public sealed class CouplingEvaluator : BaseEvaluator, IScopedDependency
{
    ...
    private async Task<IEnumerable<EvaluatorResult>> ScanDependencyManifestsAsync(
        string language, string langDir, CancellationToken token)
    {
        var manifestResults = new List<EvaluatorResult>();

         Task AddResultAsync(string file, int dependencyCount)
        {
            manifestResults.Add(new EvaluatorResult(Name, file)
            {
                Category = "Coupling",
                Metrics = new Dictionary<string, double>
                {
