...
public sealed class SecurityEvaluator : BaseArchitectureEvaluator, IScopedDependency
{
    ...
    // =====================================================
    private async Task<IEnumerable<ArchitectureEvaluatorResult>> ScanForSecretsAsync(string projectPath, CancellationToken token)
    {
        var metrics = new List<ArchitectureEvaluatorResult>();
        ...
        foreach (var file in sourceFiles)
        {
            ...
            if (secretMatches > 0)
            {
                metrics.Add(new ArchitectureEvaluatorResult(Name, file)
                {
    ...
    // =====================================================
    private async Task<IEnumerable<ArchitectureEvaluatorResult>> ScanPackageJsonAsync(string projectPath, CancellationToken token)
    {
        var metrics = new List<ArchitectureEvaluatorResult>();
                                     ...
                                     .Where(f => !IsExcludedDir(Path.GetDirectoryName(f)!)))
        {
            ...
            if (unpinnedCount > 0)
            {
                metrics.Add(new ArchitectureEvaluatorResult(Name, pkg)
                {
    ...
    // =====================================================
    private async Task<IEnumerable<ArchitectureEvaluatorResult>> ScanRequirementsAsync(string projectPath, CancellationToken token)
    {
        var metrics = new List<ArchitectureEvaluatorResult>();
                                     ...
                                     .Where(f => !IsExcludedDir(Path.GetDirectoryName(f)!)))
        {
            ...
            if (unpinned > 0)
            {
                metrics.Add(new ArchitectureEvaluatorResult(Name, req)
                {
                    Category = "SecurityDependencies",
                    Metrics = new Dictionary<string, double>
                    {
    ...
    // =====================================================
    private async Task<IEnumerable<ArchitectureEvaluatorResult>> ScanCsprojUnsafeAsync(string projectPath, CancellationToken token)
    {
        var metrics = new List<ArchitectureEvaluatorResult>();
        ...
        foreach (var csproj in csprojs)
        {
            ...
            {
                metrics.Add(new ArchitectureEvaluatorResult(Name, csproj)
                {
    ...
    // =====================================================
    private async Task<IEnumerable<ArchitectureEvaluatorResult>> ScanCryptoAndSSLAsync(string projectPath, CancellationToken token)
    {
        var metrics = new List<ArchitectureEvaluatorResult>();
